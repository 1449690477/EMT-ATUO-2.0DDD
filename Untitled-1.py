...
def wq70_wait_for_firework_drop(log_prefix: str) -> bool:
    ...
    log(
        f"{log_prefix} 同时监控 {WQ70_FIREWORK_TEMPLATE} 与 {WQ70_FIREWORK_TEMPLATE_ALT}，等待二者匹配度同步跌破 {WQ70_FIREWORK_DROP_THRESHOLD:.2f}。"
    )
    seen_primary = False
    ...
    while time.time() < deadline and not worker_stop.is_set():
        ...
        if secondary_score >= WQ70_FIREWORK_HOLD_THRESHOLD:
            ...
            if not drop_secondary:
                log(f"{log_prefix} 大烟花1匹配度下降到 {secondary_score:.3f}，等待主图同步下降。")
            drop_secondary = True
    ...
    def ensure_assets(self) -> bool:
        ...
        for name in (
            ...
            if not wq70_macro_path(name):
                missing.append(name)
        for template in (WQ70_FIREWORK_TEMPLATE, WQ70_FIREWORK_TEMPLATE_ALT):

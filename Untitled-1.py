...
class HS70AutoGUI:
    ...
    def _run_round(self, first_round: bool, prepare_next_round: bool) -> bool:
        ...
        if worker_stop.is_set():
            return False
        self._after_decrypt_actions("mapa.json", detect_target=True)
        if worker_stop.is_set():
            return False
    ...
    def _after_decrypt_actions(
        ...
        *,
        ...
        if should_detect and wait_time > 0:
            log(
                f"{self.LOG_PREFIX} 解密完成，暂停 {wait_time:.1f} 秒识别 {HS_TARGET_TEMPLATE}，阈值"

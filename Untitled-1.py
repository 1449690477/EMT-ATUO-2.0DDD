...
class AutoBloomService:
    ...
    def start(self) -> bool:
        ...
        if not self._register_hotkey():
            return False
        self.enabled = True
        log(f"自动花序弓已开启，按 {self.hotkey} 开始连发。")
        return True
...
def main():
    ...
    global auto_bloom_var, auto_bloom_hotkey_var, auto_bloom_hold_var, auto_bloom_gap_var
    ...
    auto_bloom_container = tk.Frame(toolbar)
    auto_bloom_container.pack(side="left", padx=6)
    tk.Checkbutton(
    ...
    tk.Label(auto_bloom_container, text="按(ms)").pack(side="left", padx=(6, 2))
    ttk.Entry(
        auto_bloom_container,
        width=5,
    ...
    tk.Label(auto_bloom_container, text="间(ms)").pack(side="left", padx=(6, 2))
    ttk.Entry(
        auto_bloom_container,
        width=5,

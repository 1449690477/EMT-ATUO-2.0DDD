...
def append_formatted_log(widget, message: str):
    if widget is None:
        return
    ...
    try:
        ...
        widget.insert("end", f"{icon} ", (category,))
...
class CollapsibleLogPanel(tk.Frame):
    ...
    def __init__(self, parent, title: str, text_height: int = 10):
        ...
        self.text = tk.Text(self.body, height=text_height, wrap="word")
    ...
    def append(self, message: str):
        append_formatted_log(self.text, message)
...
WQ70_EXIT_PROMPT_TEMPLATE = "撤离.png"
WQ70_EXIT_POINT_TEMPLATE = "撤离点.png"
WQ70_EXIT_THRESHOLD = 0.78
WQ70_INITIAL_MACROS = [
    "map1.json",
    "map2.json",
    "map3.json",
...
WQ70_WAIT_AFTER_MAP4_RESET = 3.0
...
def run_hs_reset_sequence(log_prefix: str, stage: str = "复位流程") -> bool:
    ...
    for template, desc, threshold, allow_tpl in sequence:
        ...
        if not path:
            ...
            return False
        if not click_template_from_path(path, f"{log_prefix} 点击 {desc}", threshold=threshold):
...
def wq70_wait(seconds: float) -> bool:
    t0 = time.time()
...
def wq70_wait_for_firework_drop(log_prefix: str) -> bool:
    template_path = os.path.join(WQ70_DIR, WQ70_FIREWORK_TEMPLATE)
    if not os.path.exists(template_path):
        log(f"{log_prefix} 缺少 {WQ70_FIREWORK_TEMPLATE}，跳过等待判定。")
        return True

    exit_prompt_path = os.path.join(WQ70_DIR, WQ70_EXIT_PROMPT_TEMPLATE)
    exit_point_path = os.path.join(WQ70_DIR, WQ70_EXIT_POINT_TEMPLATE)
    exit_assets_ready = os.path.exists(exit_prompt_path) and os.path.exists(exit_point_path)
    if not os.path.exists(exit_prompt_path):
        log(
            f"{log_prefix} 缺少 {WQ70_EXIT_PROMPT_TEMPLATE}，将仅依据大烟花匹配度下降判定。"
        )
    if not os.path.exists(exit_point_path):
        log(
            f"{log_prefix} 缺少 {WQ70_EXIT_POINT_TEMPLATE}，将仅依据大烟花匹配度下降判定。"
        )

    log(
        f"{log_prefix} 监控 {WQ70_FIREWORK_TEMPLATE}，等待匹配度低于 {WQ70_FIREWORK_DROP_THRESHOLD:.2f}。"
    )
    seen_peak = False
    best_score = 0.0
    ...
    while time.time() < deadline and not worker_stop.is_set():
        ...
        best_score = max(best_score, score)
        log(f"{log_prefix} 大烟花 匹配度 {score:.3f}")
        if score >= WQ70_FIREWORK_HOLD_THRESHOLD:
            seen_peak = True
            if drop_confirmed:
                log(f"{log_prefix} 大烟花匹配度回升到 {score:.3f}，重新等待下降。")
            drop_confirmed = False
            exit_prompt_seen = False
        elif seen_peak and score <= WQ70_FIREWORK_DROP_THRESHOLD:
            if not drop_confirmed:
                log(f"{log_prefix} 大烟花匹配度下降到 {score:.3f}，开始等待撤离提示。")
            drop_confirmed = True

        if drop_confirmed:
            if exit_assets_ready:
                exit_score, _, _ = match_template_from_path(exit_prompt_path)
                if exit_score >= WQ70_EXIT_THRESHOLD and not exit_prompt_seen:
                    log(
                        f"{log_prefix} 识别到 {WQ70_EXIT_PROMPT_TEMPLATE}（{exit_score:.3f}），继续确认撤离点。"
                    )
                    exit_prompt_seen = True
                if exit_prompt_seen:
                    exit_point_score, _, _ = match_template_from_path(exit_point_path)
                    if exit_point_score >= WQ70_EXIT_THRESHOLD and score <= WQ70_FIREWORK_DROP_THRESHOLD:
                        log(
                            f"{log_prefix} 撤离点匹配度 {exit_point_score:.3f}，且大烟花保持低匹配，继续流程。"
                        )
                        return True
            else:
                return True
    ...
    log(
        f"{log_prefix} 等待 {WQ70_FIREWORK_TIMEOUT:.0f} 秒仍未检测到匹配度下降（最高 {best_score:.3f}）。"
    )
    return False
...
class MainGUI:
    ...
    def log(self, msg: str):
        ...
        def _append():
            append_formatted_log(widget, msg)
...
class ClueFarmGUI(FragmentFarmGUI):
    ...
    def __init__(self, parent, cfg):
        ...
        self._auto_switch_origin_level = initial_level if initial_level in ("10", "30") else "10"
        ...
        self.level_var = tk.StringVar(
            value=initial_level if initial_level in ("10", "30") else "10"
        )
    ...
    def _setup_clue_ui(self):
        ...
        tip_text = (
            "没有什么好特别说明的 可以刷10 30火本 为了防止长时间刷一个本 密函的掉率降低 "
    ...
    def _anti_stuck_and_reset(self) -> bool:
        log(f"{self.log_prefix} 防卡死：ESC → G → Q → 再次进行 → 开始挑战")
        try:
            if keyboard is not None:
                keyboard.press_and_release("esc")
            else:
        ...
        if not wait_and_click_template(
            ...
            return False
        time.sleep(0.4)
        if not self._click_single_start_challenge("防卡死：开始挑战"):
            return False
        return self._map_detect_and_run_macros()

    def _map_detect_and_run_macros(self) -> bool:
        if self.level_var.get() == "30":
            return self._run_level30_macro()
        self._update_pending_decrypt_mode()
        return super()._map_detect_and_run_macros()
    ...
    def _run_level30_macro(self) -> bool:
        ...
        log(f"{self.log_prefix} 开始识别 30map（最长 12 秒）…")
        deadline = time.time() + 12.0
        best_score = 0.0
        score = 0.0
        while time.time() < deadline and not worker_stop.is_set():
        ...
        log(f"{self.log_prefix} 识别到 30map（匹配度 {score:.3f}），等待 2 秒进入宏。")
        t0 = time.time()
        while time.time() - t0 < 2.0 and not worker_stop.is_set():
            time.sleep(0.1)

        self._pending_decrypt_mode = "firework"
        self._execute_map_macro(self.level30_macro_path, "30级 地图宏")
        return True
    ...
    def _perform_mode_switch(self) -> bool:
        ...
        if wait_and_click_template(
            ...
            time.sleep(0.6)
            exit_clicked = wait_and_click_template(
        ...
        log(
            f"{self.log_prefix} 模式切换：等待索引界面加载（最多 12 秒）…"
        )
        index_ready = False
        deadline = time.time() + 12.0
        while time.time() < deadline and not worker_stop.is_set():
        ...
        if not index_ready:
            log(f"{self.log_prefix} 12 秒内未检测到 索引.png，模式切换中止。")
            return False
        ...
        current = self.level_var.get()
        target = "30" if current == "10" else "10"
        level_template = CLUE_LEVEL_30_TEMPLATE if target == "30" else CLUE_LEVEL_10_TEMPLATE
        if not wait_and_click_template(
            level_template,
    ...
    try:
        for macro in WQ70_INITIAL_MACROS:
            ...
            if macro == "map4-复位.json":
                log(
                    f"{WQ70_LOG_PREFIX} map4-复位完成，等待 {WQ70_WAIT_AFTER_MAP4_RESET:.1f} 秒再执行复位。"
                )
                if not wq70_wait(WQ70_WAIT_AFTER_MAP4_RESET):
                    return "abort"

        if not run_hs_reset_sequence(WQ70_LOG_PREFIX, "首次复位"):
            return "abort"
        if not wq70_wait(WQ70_WAIT_AFTER_RESET):
            return "abort"
        ...
        if not wq70_wait_for_firework_drop(WQ70_LOG_PREFIX):
            log(f"{WQ70_LOG_PREFIX} 未能检测到大烟花匹配度下降，本轮结束。")
            return "abort"

        if not run_hs_reset_sequence(WQ70_LOG_PREFIX, "等待完成后复位"):
            return "abort"
        if not wq70_wait(WQ70_WAIT_AFTER_RESET):
            return "abort"
        ...
        if not run_hs_reset_sequence(WQ70_LOG_PREFIX, "最终复位"):
            return "abort"
        if not wq70_wait(WQ70_WAIT_AFTER_RESET):
            return "abort"
    ...
    def ensure_assets(self) -> bool:
        ...
        for name in (
            ...
            if not wq70_macro_path(name):
                missing.append(name)
        firework_path = os.path.join(WQ70_DIR, WQ70_FIREWORK_TEMPLATE)
        if not os.path.exists(firework_path):
            missing.append(WQ70_FIREWORK_TEMPLATE)
        for template in (WQ70_EXIT_PROMPT_TEMPLATE, WQ70_EXIT_POINT_TEMPLATE):
...
class CustomScriptGUI:
    ...
    def queue_log(self, message: str):
        def _append():
            ...
            if widget is None:
                return
            append_formatted_log(widget, message)

...
DEFAULT_CONFIG = {
    ...
    "auto_bloom_toggle_hotkey": "mouse middle",
    "auto_bloom_hold_ms": 195,
    "auto_bloom_gap_ms": 50,
    "auto_skill_enabled": False,
    "auto_skill_hotkey": "f9",
    "auto_skill_e_enabled": True,
    "auto_skill_e_per_min": 12,
    "auto_skill_q_enabled": False,
    "auto_skill_q_per_min": 6,
    "wait_seconds": 8.0,
    "macro_a_path": "",
    "macro_b_path": "",
    "auto_loop": False,
...
auto_skill_e_rate_var = None
auto_skill_q_enabled_var = None
auto_skill_q_rate_var = None
...
def auto_find_game_path() -> Optional[str]:
    ...
    for drive in string.ascii_uppercase:
        ...
        if not root.exists():
            continue
        win64_dir = root / rel_dir
        if not win64_dir.exists():
            continue

        shipping_exe = next(
            (exe for exe in win64_dir.glob("*.exe") if "shipping" in exe.name.lower()),
            None,
        )
        if shipping_exe:
        ...
        all_exes = sorted(win64_dir.glob("*.exe"))
        if all_exes:
            fallback = all_exes[0]
            log(
...
class AutoBloomService:
    ...
    def __init__(self):
        ...
        self.toggle_hotkey = "mouse middle"
...
class AutoSkillService:
    ...
    def __init__(self):
        ...
        self.e_rate = 12.0
        self.q_rate = 6.0
        self.e_interval = 60.0 / self.e_rate
    ...
    def set_rates(
        self,
        e_enabled: bool,
        e_rate_per_min: float,
        q_enabled: bool,
        q_rate_per_min: float,
    ) -> bool:
        if e_enabled and e_rate_per_min <= 0:
            log("自动战斗挂机技能：E 次数必须大于 0。", level=logging.ERROR)
            return False
        if q_enabled and q_rate_per_min <= 0:
            log("自动战斗挂机技能：Q 次数必须大于 0。", level=logging.ERROR)
            return False
        self.e_enabled = bool(e_enabled and e_rate_per_min > 0)
        self.q_enabled = bool(q_enabled and q_rate_per_min > 0)
        self.e_rate = float(e_rate_per_min) if self.e_enabled else 0.0
        self.q_rate = float(q_rate_per_min) if self.q_enabled else 0.0
        self.e_interval = 60.0 / self.e_rate if self.e_enabled else None
        self.q_interval = 60.0 / self.q_rate if self.q_enabled else None
        desc = []
        if self.e_enabled:
            desc.append(f"E {self.e_rate:.1f} 次/分钟")
        if self.q_enabled:
            desc.append(f"Q {self.q_rate:.1f} 次/分钟")
        if desc:
...
def do_exit_dungeon():
    wait_and_click_template("exit_step1.png", "退图：点击 exit_step1.png", 20.0, 0.8)
    wait_and_click_template("exit_step2.png", "退图：点击 exit_step2.png", 15.0, 0.8)
...
def run_wq70_round(gui, first_round: bool) -> str:
    ...
    try:
        ...
        if not wq70_wait_for_firework_drop(WQ70_LOG_PREFIX):
            ...
            if gui is not None and not worker_stop.is_set():
                gui.log_panel.record_failure("未检测到大烟花匹配度下降，停止当前轮次。")
            return "abort"
...
def ensure_auto_bloom_service():
    ...
    if auto_bloom_service is None:
        ...
        auto_bloom_service.update_toggle_hotkey(
            cfg.get("auto_bloom_toggle_hotkey", "mouse middle")
        )
...
def on_auto_bloom_toggle():
    ...
    hotkey = auto_bloom_hotkey_var.get().strip() or "f8"
    auto_bloom_hotkey_var.set(hotkey)
    toggle_hotkey = auto_bloom_toggle_hotkey_var.get().strip() or "mouse middle"
...
def on_auto_bloom_toggle_hotkey_save():
    if auto_bloom_toggle_hotkey_var is None:
        return
    hotkey = auto_bloom_toggle_hotkey_var.get().strip() or "mouse middle"
...
def ensure_auto_skill_service() -> AutoSkillService:
    ...
    if auto_skill_service is None:
        ...
        auto_skill_service.set_rates(
            cfg.get("auto_skill_e_enabled", True),
            cfg.get("auto_skill_e_per_min", 12),
            cfg.get("auto_skill_q_enabled", False),
            cfg.get("auto_skill_q_per_min", 6),
        )
    return auto_skill_service


def _parse_auto_skill_rate(value: str, default: float) -> float:
    try:
        rate = float(value)
    except (TypeError, ValueError):
        rate = default
...
def _apply_auto_skill_settings(save: bool = True) -> bool:
    ...
    if not service.update_hotkey(hotkey):
        return False
    default_e = cfg.get("auto_skill_e_per_min", 12)
    default_q = cfg.get("auto_skill_q_per_min", 6)
    ...
    e_rate = _parse_auto_skill_rate(
        auto_skill_e_rate_var.get() if auto_skill_e_rate_var else default_e,
        default_e,
    )
    q_rate = _parse_auto_skill_rate(
        auto_skill_q_rate_var.get() if auto_skill_q_rate_var else default_q,
        default_q,
    )
    if not service.set_rates(e_enabled, e_rate, q_enabled, q_rate):
        return False
    if save and config_data is not None:
        config_data["auto_skill_hotkey"] = hotkey
        config_data["auto_skill_e_enabled"] = e_enabled
        config_data["auto_skill_e_per_min"] = e_rate
        config_data["auto_skill_q_enabled"] = q_enabled
        config_data["auto_skill_q_per_min"] = q_rate
        save_config(config_data)
    return True
...
def main():
    ...
    global auto_skill_e_enabled_var, auto_skill_e_rate_var
    global auto_skill_q_enabled_var, auto_skill_q_rate_var
    ...
    def _add_click_icon(parent):
        lbl = tk.Label(parent, image=click_icon_image, bg=link_bg, cursor="hand2")
        lbl.pack(side="left", padx=(0, 2))
        lbl.bind("<Button-1>", open_bug_feedback_link)
        return lbl

    _add_click_icon(link_row)
    ...
    auto_bloom_hotkey_var = tk.StringVar(value=cfg.get("auto_bloom_hotkey", "f8"))
    auto_bloom_toggle_hotkey_var = tk.StringVar(
        value=cfg.get("auto_bloom_toggle_hotkey", "mouse middle")
    )
    auto_bloom_hold_var = tk.StringVar(
        value=str(cfg.get("auto_bloom_hold_ms", 195))
    )
    auto_bloom_gap_var = tk.StringVar(value=str(cfg.get("auto_bloom_gap_ms", 50)))
    ...
    auto_skill_e_rate_var = tk.StringVar(
        value=str(cfg.get("auto_skill_e_per_min", 12))
    )
    auto_skill_q_enabled_var = tk.BooleanVar(
        value=cfg.get("auto_skill_q_enabled", False)
    )
    auto_skill_q_rate_var = tk.StringVar(
        value=str(cfg.get("auto_skill_q_per_min", 6))
    )
    ...
    ttk.Entry(
        auto_skill_container,
        width=4,
        textvariable=auto_skill_e_rate_var,
        justify="center",
    ).pack(side="left", padx=(2, 0))
    tk.Label(auto_skill_container, text="次/分钟").pack(side="left", padx=(2, 8))
    tk.Checkbutton(
        auto_skill_container,
        text="释放 Q", 
    ...
    ttk.Entry(
        auto_skill_container,
        width=4,
        textvariable=auto_skill_q_rate_var,
        justify="center",
    ).pack(side="left", padx=(2, 0))
    tk.Label(auto_skill_container, text="次/分钟").pack(side="left", padx=(2, 8))
    ttk.Button(

...
class MultiLetterSelectionMixin:
    ...
    def _build_letter_layout(self, frame_letters: tk.LabelFrame) -> tk.Frame:
        if not self._multi_feature_available():
            ...
            self.letters_grid_container.pack(fill="both", expand=True, padx=5, pady=(0, 5))
            self._register_letter_scroll(self.letters_grid_container)
    ...
    def _register_letter_scroll(self, widget: tk.Widget):
        if widget in self._letter_scroll_widgets:
            return
    ...
    def _on_letter_mousewheel(self, event, delta=None):
        if not getattr(self, "enable_letter_paging", False):
            return
        total = getattr(self, "total_letter_pages", 0)
        ...
        if amount > 0:
            self._prev_letter_page()
        else:
            self._next_letter_page()
        return "break"

...
class HS70AutoGUI:
    ...
    def _build_ui(self):
        ...
        notice = (
            "自动70红珠：请确保 HS 文件夹内包含初始地图、复位、分支与所有宏文件。"
            " 无巧手解密沿用人物碎片/50XP 同步逻辑，解密结束会自动等待。"
        )
        tk.Label(
            self.left_panel,
            text=notice,
    ...
    def _run_round(self, first_round: bool, prepare_next_round: bool) -> bool:
        ...
        mark_after_first = self._check_marker()
        if worker_stop.is_set():
            return False

        resolved_after_first = False
        if mark_after_first:
            log(f"{self.LOG_PREFIX} 第一段识别到标记，准备执行校准宏。")
            ...
            self.set_detail("首段已完成校准，跳过后续开锁。")
        else:
            log(f"{self.LOG_PREFIX} 第一段未识别到标记，继续执行第二段开锁。")

        mark_after_second = False
        if not resolved_after_first:
            ...
            mark_after_second = self._check_marker()
            if worker_stop.is_set():
                return False
            if mark_after_second:
                log(f"{self.LOG_PREFIX} 第二段识别到标记，准备执行校准宏。")
                if not self._run_macro_with_decrypt("mapa-开锁2-校准.json"):
                    self.set_status("执行 mapa-开锁2-校准.json 失败")
                    return False
            else:
                log(f"{self.LOG_PREFIX} 第二段仍未识别到标记，进入分支识别流程。")
    ...
    def _run_macro_with_decrypt(self, macro_name: str, segment_key: Optional[str] = None) -> bool:
    ...
    def _check_marker(self) -> bool:
        path = hs_find_asset(HS_MARK_TEMPLATE)
        if not path:
        ...
        log(
            ...
            f" {self.MARK_CONFIRMATIONS} 次命中。"
        )
        self.set_detail("检测标记…")
        end = time.time() + self.MARK_SCAN_DURATION
        best = 0.0
        streak = 0
        while time.time() < end and not worker_stop.is_set():
            score, _, _ = match_template_from_path(path)
            best = max(best, score)
            if score >= self.MARK_THRESHOLD:
                streak += 1
                log(
                    f"{self.LOG_PREFIX} 标记检测命中，得分 {score:.3f}，连续 {streak}/"
                    f"{self.MARK_CONFIRMATIONS}"
                )
                if streak >= self.MARK_CONFIRMATIONS:
                    log(
                        f"{self.LOG_PREFIX} 标记检测成功，最终得分 {score:.3f}，准备执行校准宏。"
                    )
                ...
                continue
            streak = 0
            time.sleep(self.MARK_SCAN_INTERVAL)

        log(
            f"{self.LOG_PREFIX} 标记检测未成功，最高得分 {best:.3f} < {self.MARK_THRESHOLD:.2f}。"
        )
    ...
    def _scan_branch_variation(self) -> bool:
        ...
        while time.time() < end and not worker_stop.is_set():
            for label, info in HS_BRANCH_OPTIONS.items():
                tpl_path = hs_find_asset(info["template"])
                if not tpl_path:
                    continue
                score, _, _ = match_template_from_path(tpl_path)
                log(f"{self.LOG_PREFIX} 分支 {label} 匹配度 {score:.3f}")
                if score > best_score:
                    best_score = score
                    best_label = label
        ...
        self.set_detail(
            f"识别到 {best_label} 分支（匹配度 {best_score:.3f}），执行对应宏。"
        )
    ...
    def _click_template_from_path(
        ...
        *,
        ...
        for attempt in range(1, attempts + 1):
            score, x, y = match_template_from_path(path)
            log(f"{self.LOG_PREFIX} {label} 匹配[{attempt}/{attempts}] {score:.3f}")
            if score >= threshold and x is not None:
                if perform_click(x, y):
                    time.sleep(0.3)
                    return True

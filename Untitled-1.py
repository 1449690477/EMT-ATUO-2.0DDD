...
class HS70AutoGUI:
    ...
    def _build_ui(self):
        ...
        if tip_path:
            ...
            if photo is not None:
                tk.Label(notice_frame, image=photo).pack(side="left", padx=(8, 0))
    ...
    def _run_compensation_after_calibration(self):
        if worker_stop.is_set():
            return
        path = hs_find_asset(HS_COMPENSATE_MACRO)
        if not path:
            log(f"{self.LOG_PREFIX} 缺少 {HS_COMPENSATE_MACRO}，无法执行校准补偿。")
            return
        log(f"{self.LOG_PREFIX} 校准完成，执行补偿宏 {HS_COMPENSATE_MACRO}。")
        self.set_detail("校准后执行补偿宏…")
        previous_last = self.last_macro_name
        success = self._run_macro_with_decrypt(HS_COMPENSATE_MACRO, require_decrypt=False)
        self.last_macro_name = previous_last
        if not success:
            log(f"{self.LOG_PREFIX} 补偿宏 {HS_COMPENSATE_MACRO} 执行失败。")
    ...
    def _run_round(self, first_round: bool, prepare_next_round: bool) -> bool:
        ...
        if first_target:
            ...
            if worker_stop.is_set():
                return False
            self._run_compensation_after_calibration()
            if worker_stop.is_set():
                return False

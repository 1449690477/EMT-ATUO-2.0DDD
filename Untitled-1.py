...
DEFAULT_CONFIG = {
    ...
    "auto_skill_e_count": 12.0,
    "auto_skill_e_period": 60.0,
    "auto_skill_q_enabled": False,
    "auto_skill_q_count": 6.0,
    "auto_skill_q_period": 60.0,
    "wait_seconds": 8.0,
    "macro_a_path": "",
    "macro_b_path": "",
    "auto_loop": False,
...
def ensure_auto_skill_service() -> AutoSkillService:
    ...
    if auto_skill_service is None:
        ...
        if e_count is None:
            e_count = cfg.get("auto_skill_e_per_min", 12.0)
            e_period = 60.0
        else:
            e_period = cfg.get("auto_skill_e_period", 60.0)
        q_count = cfg.get("auto_skill_q_count")
        if q_count is None:
            q_count = cfg.get("auto_skill_q_per_min", 6.0)
            q_period = 60.0
        else:
            q_period = cfg.get("auto_skill_q_period", 60.0)
...
def _apply_auto_skill_settings(save: bool = True) -> bool:
    ...
    if default_e_count is None:
        default_e_count = cfg.get("auto_skill_e_per_min", 12.0)
        default_e_period = 60.0
    else:
        default_e_period = cfg.get("auto_skill_e_period", 60.0)
    default_q_count = cfg.get("auto_skill_q_count")
    if default_q_count is None:
        default_q_count = cfg.get("auto_skill_q_per_min", 6.0)
        default_q_period = 60.0
    else:
        default_q_period = cfg.get("auto_skill_q_period", 60.0)
...
def main():
    ...
    auto_skill_e_count_var = tk.StringVar(
        value=str(
            cfg.get(
                "auto_skill_e_count",
                cfg.get("auto_skill_e_per_min", 12.0),
            )
        )
    )
    auto_skill_e_period_var = tk.StringVar(
        value=str(cfg.get("auto_skill_e_period", 60.0))
    )
    ...
    auto_skill_q_count_var = tk.StringVar(
        value=str(
            cfg.get(
                "auto_skill_q_count",
                cfg.get("auto_skill_q_per_min", 6.0),
            )
        )
    )
    auto_skill_q_period_var = tk.StringVar(
        value=str(cfg.get("auto_skill_q_period", 60.0))
    )

...
class HS70AutoGUI:
    ...
    MARK_SCAN_DURATION = 1.0
    MARK_SCAN_INTERVAL = 0.1
    ...
    def _run_round(self, first_round: bool, prepare_next_round: bool) -> bool:
        ...
        if not self._identify_submap():
            return False
        if worker_stop.is_set():
            return False

        if not self._run_macro_with_decrypt(HS_FINAL_MACRO, segment_key="final_macro"):
            self.set_status("执行 复位撤离.json 失败")
            return False
    ...
    def _ensure_branch_a(self) -> bool:
        ...
        log(
            f"{self.LOG_PREFIX} 分支A 匹配失败，最高 {best:.3f} < {self.BRANCH_THRESHOLD:.2f}，执行防卡死。"
        )
        ...
        while time.time() < end and not worker_stop.is_set():
            for label, info in HS_BRANCH_OPTIONS.items():
                ...
                if score > best_score:
                    best_score = score
                    best_label = label
            if best_label and best_score >= self.BRANCH_THRESHOLD:
                break
            time.sleep(self.BRANCH_SCAN_INTERVAL)

        if best_label is None:
            log(f"{self.LOG_PREFIX} 未能识别到 2-3/2-4 分支。")
    ...
    def _identify_submap(self) -> bool:

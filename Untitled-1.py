...
class HS70AutoGUI:
    ...
    def _hold_key_with_check(self, key: str, duration: float):
        if duration <= 0:
            return
        if keyboard is None and pyautogui is None:
        ...
        try:
            ...
            log(f"{self.LOG_PREFIX} 长按 {key} 失败：{exc}")
        finally:
            if pressed:
                try:
                    if keyboard is not None:
    ...
    def _post_calibration_adjustment(self):
        if worker_stop.is_set():
            return
        hold = max(0.0, self.POST_CALIBRATION_HOLD)
        if hold == 0:
            return
        log(
            f"{self.LOG_PREFIX} 校准后执行调整：按住 S {hold:.1f} 秒 → D {hold:.1f} 秒。"
        )
        self.set_detail("校准后调整：按住 S → D。")
        self._hold_key_with_check("s", hold)
        if worker_stop.is_set():
            return
        self._sleep_with_check(0.05)
        if worker_stop.is_set():
            return
        self._hold_key_with_check("d", hold)
    ...
    def _run_round(self, first_round: bool, prepare_next_round: bool) -> bool:
        ...
        if first_target:
            ...
            if worker_stop.is_set():
                return False
            self._post_calibration_adjustment()
            if worker_stop.is_set():
                return False

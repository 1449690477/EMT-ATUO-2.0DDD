...
def wq70_wait_for_firework_drop(log_prefix: str) -> bool:
    ...
    while time.time() < deadline and not worker_stop.is_set():
        ...
        if score >= WQ70_FIREWORK_HOLD_THRESHOLD:
            ...
            log(f"{log_prefix} 大烟花匹配度下降到 {score:.3f}，继续流程。")
            return True
        time.sleep(0.4)
...
class ClueFarmGUI(FragmentFarmGUI):
    ...
    def _setup_clue_ui(self):
        ...
        if hasattr(self, "top_tip_label") and self.top_tip_label.winfo_exists():
            self.top_tip_label.config(text=clue_top_tip)
        else:
            self.top_tip_label = tk.Label(
                self.parent,
                text=clue_top_tip,
                fg="red",
                font=("Microsoft YaHei", 10, "bold"),
            )
            self.top_tip_label.pack(fill="x", padx=10, pady=3)

...
class FragmentFarmGUI(MultiLetterSelectionMixin):
    ...
    def _build_ui(self):
        tip_top = tk.Label(
            self.parent,
            text="没有什么好说明的 直接用就行 支持10 30火本 为了防止长时间挂机 爆率降低 我加入了 自动切换模式等级功能 默认是1小时切换一次",
            fg="red",
            font=("Microsoft YaHei", 10, "bold"),
        )
...
class ClueFarmGUI(FragmentFarmGUI):
    ...
    def _setup_clue_ui(self):
        ...
        if hasattr(self, "top_tip_label"):
            self.top_tip_label.config(
                text="没什么好说明的。。我给这个页面增加了自定义时间切换10 和 30 火本的功能 防止掉落率降低 默认是60分钟 。"
            )
    ...
    def _perform_mode_switch(self) -> bool:
        ...
        if wait_and_click_template(
            "G.png",
            f"{self.log_prefix} 模式切换：点击 G.png",
            2.0,
            0.6,
        ):
            time.sleep(0.6)
            click_template("Q.png", f"{self.log_prefix} 模式切换：点击 Q.png", 0.6)
            time.sleep(0.6)
            ...
            log(
                f"{self.log_prefix} 模式切换：未在 2 秒内识别到 G.png，尝试撤退退出。"
            )
            if wait_and_click_template(
                BTN_RETREAT_START,
        ...
        time.sleep(0.6)
        if not wait_and_click_template(
            CLUE_SETTINGS_TEMPLATE,
            f"{self.log_prefix} 模式切换：打开设置",
            20.0,
            0.75,
        ):
            log(f"{self.log_prefix} 模式切换：未能点击 设置.png。")
            return False
        time.sleep(0.6)
        for tpl, desc in (
            (CLUE_TRAINING_TEMPLATE, "设置 → 历练"),
...
def main():
    ...
    experimental_monitor_var = tk.BooleanVar(value=cfg.get("experimental_monitor_enabled", False))
    ...
    tk.Checkbutton(
        toolbar,
        text="实验性程序",
        variable=experimental_monitor_var,
        command=on_experimental_toggle,
    ).pack(side="left", padx=4)
